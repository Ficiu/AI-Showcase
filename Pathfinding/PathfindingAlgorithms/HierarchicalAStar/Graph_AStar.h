...
#include "RTSCommon/AISystem/GraphSystem/Graph.h"
#include "RTSCommon/AISystem/Utilities/PriorityQueue.h"
#include <queue>
#include <vector>
#include <glm/glm.hpp>
#include <ctime>
...
public:
	AStar(const graph_type& graph, glm::vec2 cellSize, int source, int target = -1) :
		graph(graph), sourceNode(source), targetNode(target), shortestPathTree(graph.GetNumNodes()),
		realCostTo(graph.GetNumNodes()), frontier(graph.GetNumNodes()), FCost(graph.GetNumNodes())
	{
		tieBreaker = 1.01;
	...
	AStar(const graph_type& graph, glm::vec2 cellSize, glm::vec2 topleft, glm::vec2 bottomright, int source, int target = -1) :
		graph(graph), sourceNode(source), targetNode(target), shortestPathTree(graph.GetNumNodes()),
		realCostTo(graph.GetNumNodes()), frontier(graph.GetNumNodes()), FCost(graph.GetNumNodes()),
		clusterTopLeft(topleft), clusterBottomRight(bottomright)
	{
		tieBreaker = 1.01;
		diagonalFactor = glm::length(cellSize) - (cellSize.x + cellSize.y);
...
{
	int dx = std::fabs((graph.GetNode(targetNode).GetNodePosition().x -
		graph.GetNode(node).GetNodePosition().x));

	int dy = std::fabs((graph.GetNode(targetNode).GetNodePosition().y -
